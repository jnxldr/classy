...
def google_read(spreadsheet_id, spreadsheet_source):
    ...
    result = sheet.values().get(spreadsheetId=spreadsheet_id,
                                range=spreadsheet_source).execute()

    addon_list = result.get('values', [])
    print('{0} addons retrieved.'.format(len(addon_list)))
...
def google_write(spreadsheet_id, spreadsheet_target):
    ...
    sheet.values().update(spreadsheetId=spreadsheet_id,
                          range=spreadsheet_target).execute()
